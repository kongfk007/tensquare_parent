...
public class RecruitController {
	...
	public Result pageQuery(@RequestBody Recruit recruit,@PathVariable int page,@PathVariable int size){
		Page<Recruit>  pageData = recruitService.pageQuery(recruit,page,size);
		System.out.println(pageData);
		return new Result(true,StatusCode.OK,"查询成功",pageData);
//				new PageResult<Recruit>(pageData.getTotalElements(),pageData.getContent()));
	}
	 ...
	 * @return 分页结果
	 */
	@RequestMapping(value="/search2/{page}/{size}",method=RequestMethod.POST)
